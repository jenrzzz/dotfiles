...
# shellcheck disable=SC2034  # TMUX_POWERLINE_* vars are consumed externally by the powerline framework
# Shared powerline layout for the jenner + meta-* themes. This is NOT meant to be selected
# directly: each real theme is a thin wrapper that sets a palette and then sources this file.
# Every accent below reads from a META_* palette variable that defaults to jenner's original
# value, so sourcing with no palette set reproduces the local (blue) look byte-for-byte.
#
# Palette knobs (defaults = jenner):
#   META_BG         234  default background
#   META_ACCENT      74  default foreground / accent
#   META_WIN_ACCENT  74  active-window title accent inside the window-status formats
#   META_FLEET_BG   233  claude_fleet segment background (left edge)
#   META_FLEET_FG    39  claude_fleet segment foreground
#   META_HOST_BG     23  hostname segment background (right edge — primary host cue)
#   META_HOST_FG    255  hostname segment foreground
#
# If changes made here does not take effect, then try to re-create the tmux session to force reload.

# Palette defaults (jenner / local look) — variant wrappers override these before sourcing.
: "${META_BG:=234}"
: "${META_ACCENT:=74}"
: "${META_WIN_ACCENT:=74}"
: "${META_FLEET_BG:=233}"
...
TMUX_POWERLINE_DEFAULT_BACKGROUND_COLOR=${TMUX_POWERLINE_DEFAULT_BACKGROUND_COLOR:-"$META_BG"}
TMUX_POWERLINE_DEFAULT_FOREGROUND_COLOR=${TMUX_POWERLINE_DEFAULT_FOREGROUND_COLOR:-"$META_ACCENT"}
...
if [ -z "$TMUX_POWERLINE_WINDOW_STATUS_CURRENT" ]; then
	TMUX_POWERLINE_WINDOW_STATUS_CURRENT=(
		...
		"  #I#{?window_bell_flag,#[fg=colour220]▴#[fg=colour${META_WIN_ACCENT}],#{?window_flags,#F, }}"
		"$TMUX_POWERLINE_SEPARATOR_RIGHT_THIN"
...
if [ -z "$TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS" ]; then
	...
	TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS+=(
		...
		# right_disable strips powerline's trailing space; the three tokens before it restore the
		# default separator/colors so only the spacing changes (they're required for index alignment).
		"hostname ${META_HOST_BG} ${META_HOST_FG} default_separator no_sep_bg_color no_sep_fg_color right_disable"
	)
